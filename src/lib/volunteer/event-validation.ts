...
export type SlotErrors = Partial<Record<"role_name" | "capacity" | "timing", string>>;
...
export function validateSlot(slot: VolunteerSlotDraft, event: EventDraft): SlotErrors {
  ...
  // VolunteerSlotDraft type comment) — SlotEditor picks one `date` for the
  // whole slot, so there is only one field to read here, unlike the event's
  // own start_date/end_date pair. effectiveSlotDate resolves a blank/stale
  // `date` to the event's own day when the event only spans one — see that
  // function's comment for why a slot can't always be trusted to have this
  // set for itself (e.g. right after "Duplicate event").
